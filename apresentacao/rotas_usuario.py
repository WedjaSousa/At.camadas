...
def criar_usuario():
    ...
    try:
        ...
        telefone = dados.get('telefone')  # Campo telefone adicionado
        
        if not nome or not email:
            return jsonify({"erro": "Nome e email são obrigatórios"}), 400
        
        # Chamar camada de serviço repassando o telefone
        usuario = servico_usuario.criar_usuario(nome, email, telefone)
        
        # Retornar resposta formatada com o campo telefone
        return jsonify({
            "id": usuario.id,
            "nome": usuario.nome,
            "email": usuario.email,
...
def atualizar_usuario(usuario_id):
    ...
    try:
        ...
        nome = dados.get('nome')
        email = dados.get('email')
        telefone = dados.get('telefone')  # Extração do telefone para atualização
