...
class RepositorioUsuario:
    ...
    def salvar(self, usuario: Usuario) -> Usuario:
        ...
        try:
            # Executar INSERT incluindo a coluna telefone e o quarto caractere '?'
            cursor.execute(
                """
    ...
    def atualizar(self, usuario: Usuario) -> Usuario:
        ...
        try:
            cursor.execute(
                """
                UPDATE usuarios
                SET nome = ?, email = ?, ativo = ?, telefone = ?
                WHERE id = ?
                """,
                (usuario.nome, usuario.email, usuario.ativo, usuario.telefone, usuario.id)
            )
    ...
    def _converter_linha_para_usuario(linha: tuple) -> Usuario:
        ...
        Args:
            linha: Tupla com dados do banco (id, nome, email, ativo, telefone)
        ...
        # Criar entidade de domínio passando também o telefone desempacotado
        usuario = Usuario(
            nome=nome,
            email=email,
            ativo=bool(ativo),
...
if __name__ == "__main__":
    ...
    u = Usuario(nome="Maria Repositorio", email="maria_repo@email.com", telefone="81999999999")
    usuario_salvo = repo.salvar(u)
    usuario_buscado = repo.buscar_por_id(usuario_salvo.id)
    print("\n🚀 RESULTADO DO TESTE DO BANCO DE DADOS:")
    print(usuario_buscado.telefone)