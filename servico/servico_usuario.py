...
class ServicoUsuario:
    ...
    def criar_usuario(self, nome: str, email: str, telefone: Optional[str] = None) -> Usuario:
        """
        ...
        # Criar entidade de domínio (valida dados automaticamente) incluindo telefone
    ...
    def atualizar_usuario(self, usuario_id: int, nome: str = None, 
        ...
        if telefone:
            # Garante que a sua entidade aceita a atualização de telefone se houver o método
            if hasattr(usuario, 'atualizar_telefone'):
                usuario.atualizar_telefone(telefone)
            else:
...
if __name__ == "__main__":
    # Teste rápido de integração da camada de Serviço
    try:
        servico = ServicoUsuario()
        # Criando usuário com e-mail único usando um número aleatório para evitar colisão
        import random
        email_teste = f"teste_servico{random.randint(100, 999)}@email.com"
        
        u_salvo = servico.criar_usuario(
            nome="Anderson Servico", 
            email=email_teste, 
            telefone="81988888888"
        )
        print("\n✨ RESULTADO DO TESTE DE SERVIÇO (ATIVIDADE 4):")
        print(f"Usuário criado via Serviço com Sucesso! Telefone: {u_salvo.telefone}")
    except Exception as e:
        print(f"\n❌ Erro ao rodar teste de Serviço: {e}")